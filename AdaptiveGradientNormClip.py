...
class AdaptiveGradNormClip(object):
	...
	Essentially, just a running buffer of gradient norms from the last n gradient steps,
    with a hook into the x-th percentile of those values, which is intended to be used to 
    set the ceiling on the gradient applied at the next iteration of 
    a gradient-descent-like procedure.
	...
	def __init__(self,
		...
		Args:
			...
			-do_adaptive_clipping: A bool indicating whether to implement adaptive gradient norm clipping 
            (i.e., the purpose of this class). Setting to False leads to clipping at a fixed gradient norm 
            specified by fixed_clip_val. Default: True
