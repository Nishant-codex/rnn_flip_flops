...
class FixedPointStore(object):
  ...
  def __init__(self, 
    ...
    if alloc_zeros == True:
      ...
      if xstar is not None:
        self.n, self.num_states = xstar.shape
  ...
  def get_unique(self):
    ...
    # print(self[idx[0]].num_inits)
    print(self[idx[0]].num_inputs)
    # print(self[idx[0]].num_states)

    # fps = FixedPointsStore(num_inits=self[idx[0]].num_inits,
    #                        num_inputs=self[idx[0]].num_inputs,
    #                        num_states = self[idx[0]].num_states)
    # print(idx)
    # for i in idx:
    #   # print('Im here')
    #   print(self[i].num_inputs)
    #   fps[i] = self[i]
    return self[idx]
  
  def exclude_distance_outliers():
    pass
  ...
  def __getitem__(self, index):
    xstar  = self.index(self.xstar , index) 
    x_init = self.index(self.x_init, index)
    inputs = self.index(self.inputs, index)
    F_xstar= self.index(self.F_xstar, index)
    qstar  = self.index(self.qstar, index)
    dq = self.index(self.dq, index)
    n_iters = self.index(self.n_iters, index)
    tol = self.tol_unique
    J_xstar = self.index(self.J_xstar, index)
    ...
    if self.has_decomposed_jacobians:
      ...
      eigval_J_xstar = None
      eigvec_J_xstar = None
    indexed_fp = FixedPointStore(num_inits=self.num_inits,
                             num_states=self.num_states,
