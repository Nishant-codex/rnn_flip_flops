...
def plot_fps(fps,
		...
		if state_traj is not None:
				...
				# print(state_traj_bxtxd.shape)
				[n_time, n_batch, n_states] = state_traj_bxtxd.shape
		...
		plt.show()
		plt.pause(1e-10)
		plt.savefig('outputsw_unstable_w_hid.png')

def plot_fixed_point(ax, fp, pca,
	scale= 1.0,
	max_n_modes= 3,
	do_plot_unstable_fps= True,
	do_plot_stable_modes= True, # (for unstable FPs)
	stable_color= 'k',
	stable_marker= '.',
	unstable_color= 'r',
	unstable_marker='.',
	**kwargs):
	...
	xstar = fp.xstar
	J = fp.J_xstar
	n_states = fp.num_states

	has_J = J is not None
	# print('has j val', has_J)
	if has_J:
