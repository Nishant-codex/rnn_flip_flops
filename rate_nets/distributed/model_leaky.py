...
class LeakyRNNCell(RNNCell):
    ...
    def call(self, inputs, state):
        ...
        if self._w_rec_init == 'exc_inh':
          rnn = math_ops.matmul(state,(par['EI_matrix']@ tf.math.abs(self._kernel[par['n_output']:,:])))
        else:
...
def main(gpu_id = None):
    ...
    with tf.Session(config=config) as sess:
        ...
        w_out = sess.run(model.var_dict)
        save_results(model_performance, w_out, kernel, h)        
        path = os.getcwd()
        # model.save()
        plt.plot(loss_)
        plt.show()
...
def save_results(model_performance, weights, kernel ,h, save_fn = None):
    ...
    sio.savemat(fn_rnn,{'w_in':w_in,'w_out':w_out,'w_rnn':w_rnn,'hid':h})
    # sio.savemat(fn_in)
...
def transition_graph(cell, sess, plot=True, save=True):
  ...
  if save:
    save_transitions(trans,os.getcwd()+'/fps_saver')
