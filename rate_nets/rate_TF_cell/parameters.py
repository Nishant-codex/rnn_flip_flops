...
def update_dependencies():
    ...
    w = np.zeros((par['n_hidden'],par['n_hidden']), dtype = np.float32)
    idx = np.where(np.random.rand(par['n_hidden'], par['n_hidden']) < par['connection_prob'])
    if par['dist'] == 'gamma':
        w[idx[0], idx[1]] = np.random.gamma(2, 0.003, len(idx[0]))
    elif par['dist']  == 'gauss':
        w[idx[0], idx[1]] = np.random.normal(0, 1.0, len(idx[0]))
        w = w/np.sqrt(par['n_hidden']*par['connection_prob'])*1.5# scale by a gain to make it chaotic
    ...
    if par['EI']:
        ...
        if par['balance_EI']:
            ...
            par['w_rnn0'][:, par['ind_inh']] = initialize([par['n_hidden'], par['num_inh_units']], par['connection_prob'], shape=0.5, scale=1.)
            par['w_rnn0'][par['ind_inh'], :] = initialize([par['num_inh_units'], par['n_hidden']], par['connection_prob'], shape=0.5, scale=1.)
