...
class LeakyRNNCell(RNNCell):
    ...
    def __init__(self,
        ...
        if self._w_rec_init == 'diag':
            ...
            num_exc_units = int(np.round(n_hidden*0.8))
            ...
            w_rec0 = self.initialize([n_hidden, n_hidden], 1.)

            w_rnn_mask = np.ones_like(w_rnn0)
            
            w_rnn_mask = np.ones((n_hidden,n_hidden), dtype=np.float32) - np.eye(n_hidden)
...
class Model:
    ...
    def run_dyn_rnn(self):
        ...
        self.y = tf.tensordot(self.h,tf.nn.relu(self.var_dict['w_out'] ),axes=1) + self.var_dict['b_out']
...
def fixed_points(hid,cell,sess):
  ...
  fps.sample_states(600,hid,'rate',0.5)
